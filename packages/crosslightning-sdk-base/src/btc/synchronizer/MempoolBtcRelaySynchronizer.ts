import {
  ...
  BtcStoredHeader,
  RelaySynchronizer,
} from '@onenesslabs/crosslightning-base/dist';
import { MempoolBitcoinBlock } from '../MempoolBitcoinBlock';
import { MempoolBitcoinRpc } from '../MempoolBitcoinRpc';
import { ChainUtils } from '../ChainUtils';

export class MempoolBtcRelaySynchronizer<B extends BtcStoredHeader<any>, TX>
  implements RelaySynchronizer<B, TX, MempoolBitcoinBlock>
{
  ...
  async syncToLatestTxs(): Promise<{
    ...
    const { resultStoredHeader, resultBitcoinHeader } =
      await this.btcRelay.retrieveLatestKnownBlockLog();
    ...
    console.log(
      'Retrieved stored header with commitment: ',
      cacheData.lastStoredHeader
    );
    ...
    const saveHeaders = async (headerCache: MempoolBitcoinBlock[]) => {
      ...
      if (cacheData.forkId === -1) {
        cacheData = await this.btcRelay.saveNewForkHeaders(
          headerCache,
          cacheData.lastStoredHeader,
          tipData.chainWork
        );
      } else if (cacheData.forkId === 0) {
        cacheData = await this.btcRelay.saveMainHeaders(
          headerCache,
          cacheData.lastStoredHeader
        );
      } else {
        cacheData = await this.btcRelay.saveForkHeaders(
          headerCache,
          cacheData.lastStoredHeader,
          cacheData.forkId,
          tipData.chainWork
    ...
    while (retrievedHeaders == null || retrievedHeaders.length > 0) {
      retrievedHeaders = (
        await ChainUtils.getPast15Blocks(spvTipBlockHeight + 15)
      ).map((e) => new MempoolBitcoinBlock(e));
