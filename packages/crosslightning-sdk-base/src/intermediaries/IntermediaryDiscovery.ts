...
import { SwapContract } from '@onenesslabs/crosslightning-base';
...
function getIntermediaryComparator(
  swapType: SwapType,
  ...
  return (a: Intermediary, b: Intermediary): number => {
    if (swapAmount == null) {
      return new BN(a.services[swapType].swapFeePPM)
        .sub(new BN(b.services[swapType].swapFeePPM))
        .toNumber();
    } else {
      const feeA = new BN(a.services[swapType].swapBaseFee).add(
        swapAmount
          .mul(new BN(a.services[swapType].swapFeePPM))
          .div(new BN(1000000))
      );
      const feeB = new BN(b.services[swapType].swapBaseFee).add(
        swapAmount
          .mul(new BN(b.services[swapType].swapFeePPM))
          .div(new BN(1000000))
      );
...
export class IntermediaryDiscovery<T extends SwapData> {
  ...
  async getNodeInfo(
  ...
  ): Promise<{ address: string; info: InfoHandlerResponseEnvelope }> {
    ...
    await this.swapContract.isValidDataSignature(
      Buffer.from(jsonBody.envelope),
      jsonBody.signature,
      jsonBody.address
    );
  ...
  async getReputation(node: {
    ...
    for (let token of checkReputationTokens) {
      promises.push(
        tryWithRetries(() =>
          this.swapContract.getIntermediaryReputation(
            node.address,
            this.swapContract.toTokenAddress(token)
          )
        ).then((result) => {
  ...
  async init(abortSignal?: AbortSignal) {
    ...
    for (let url of urls) {
      promises.push(
        this.getNodeInfo(url, abortSignal)
          .then((resp) => {
            activeNodes.push({
    ...
    for (let node of activeNodes) {
      //Fetch reputation
      promises.push(
        this.getReputation(node).then((reputation) => {
          const services: ServicesType = {};

          for (let key in node.info.services) {
            services[swapHandlerTypeToSwapType(key as SwapHandlerType)] =
              node.info.services[key];
          }

          this.intermediaries.push(
            new Intermediary(node.url, node.address, services, reputation)
          );
        })
      );
  ...
  getSwapMinimum(swapType: SwapType): number {
    ...
    this.intermediaries.forEach((intermediary) => {
      ...
      if (swapService != null) {
        min == null
          ? (min = swapService.min)
          : (min = Math.min(min, swapService.min));
      }
    });
    return min;
  }
  ...
  getSwapMaximum(swapType: SwapType): number {
    ...
    this.intermediaries.forEach((intermediary) => {
      ...
      if (swapService != null) {
        max == null
          ? (max = swapService.max)
          : (max = Math.max(max, swapService.max));
      }
    });
    return max;
  }
