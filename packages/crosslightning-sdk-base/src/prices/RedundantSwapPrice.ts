...
export class RedundantSwapPrice extends ISwapPrice {
  static create(
    ...
    const priceApis = [
      new BinancePriceProvider(
        BinanceSwapPrice.createCoinsMap(wbtcAdress, usdcAddress, usdtAddress)
      ),
      new OKXPriceProvider(
        OKXSwapPrice.createCoinsMap(wbtcAdress, usdcAddress, usdtAddress)
      ),
      new CoinGeckoPriceProvider(
        CoinGeckoSwapPrice.createCoinsMap(wbtcAdress, usdcAddress, usdtAddress)
      ),
      new CoinPaprikaPriceProvider(
        CoinPaprikaPriceProvider.createCoinsMap(
          wbtcAdress,
          usdcAddress,
          usdtAddress
        )
    ...
    return new RedundantSwapPrice(
      maxAllowedFeeDiffPPM,
      RedundantSwapPrice.createCoinsMap(wbtcAdress, usdcAddress, usdtAddress),
      priceApis,
      cacheTimeout
    );
  }
  ...
  static createFromTokens(
    ...
    const priceApis = [
      new BinancePriceProvider(
        BinanceSwapPrice.createCoinsMapFromTokens(tokens, nativeTokenTicker)
      ),
      new OKXPriceProvider(
        OKXSwapPrice.createCoinsMapFromTokens(tokens, nativeTokenTicker)
      ),
      new CoinGeckoPriceProvider(
        CoinGeckoSwapPrice.createCoinsMapFromTokens(tokens, nativeTokenTicker)
      ),
      new CoinPaprikaPriceProvider(
        CoinPaprikaPriceProvider.createCoinsMapFromTokens(
          tokens,
          nativeTokenTicker
        )
      ),
    ...
    return new RedundantSwapPrice(
      maxAllowedFeeDiffPPM,
      RedundantSwapPrice.createCoinsMapFromTokens(tokens, nativeTokenTicker),
      priceApis,
      cacheTimeout
    );
  }
  ...
  constructor(
    maxAllowedFeeDiffPPM: BN,
    coinsMap: { [key: string]: number },
    priceApis: IPriceProvider[],
    cacheTimeout?: number
  ) {
  ...
  getMaybeOperationalPriceApis(): {
    ...
    let operational = this.priceApis.filter(
      (e) => e.operational === true || e.operational === null
    );
    ...
    return tryWithRetries(
      () => {
        ...
        if (operationalPriceApi != null) {
          return operationalPriceApi.priceApi
            .getPrice(token, abortSignal)
            .catch((err) => {
              ...
              return promiseAny(
                this.getMaybeOperationalPriceApis().map((obj) =>
                  obj.priceApi
                    .getPrice(token, abortSignal)
                    .then((price) => {
                ...
                Promise.reject(
                  e.find((err) => !(err instanceof HttpResponseError)) || e[0]
                )
              );
            });
        }
      ...
      null,
      (e) => e instanceof HttpResponseError,
      abortSignal
    );
  }
  ...
  async getPrice(token: TokenAddress, abortSignal?: AbortSignal): Promise<BN> {
    ...
    if (thisFetch != null) {
      ...
      price = await this.cache[token].price.catch((e) =>
        this._getPrice(token, abortSignal)
      );
    }
  ...
  async getFromBtcSwapAmount(
    ...
    if (this.COINS_MAP[toToken.toString()] == null)
      throw new Error('Token not found!');

    const price =
      preFetchedPrice || (await this.getPrice(toToken, abortSignal));
  ...
  async getToBtcSwapAmount(
    ...
    if (this.COINS_MAP[fromToken.toString()] == null)
      throw new Error('Token not found');

    const price =
      preFetchedPrice || (await this.getPrice(fromToken, abortSignal));
