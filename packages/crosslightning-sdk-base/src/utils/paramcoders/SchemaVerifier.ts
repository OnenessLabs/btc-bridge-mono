...
export type FieldType<
  T extends
    | FieldTypeEnum
    | RequestSchema
    | ((val: any) => string | boolean | number | BN | any)
> = T extends FieldTypeEnum.String
  ? string
  : T extends FieldTypeEnum.Boolean
  ? boolean
  : T extends FieldTypeEnum.Number
  ? number
  : T extends FieldTypeEnum.BN
  ? BN
  : T extends FieldTypeEnum.Any
  ? any
  : T extends FieldTypeEnum.StringOptional
  ? string
  : T extends FieldTypeEnum.BooleanOptional
  ? boolean
  : T extends FieldTypeEnum.NumberOptional
  ? number
  : T extends FieldTypeEnum.BNOptional
  ? BN
  : T extends FieldTypeEnum.AnyOptional
  ? any
  : T extends RequestSchema
  ? RequestSchemaResult<T>
  : T extends (val: any) => string
  ? string
  : T extends (val: any) => boolean
  ? boolean
  : T extends (val: any) => number
  ? number
  : T extends (val: any) => BN
  ? BN
  : T extends (val: any) => any
  ? any
  : never;
...
export function verifyField<
  T extends
    ...
    | RequestSchema
    | ((val: any) => string | boolean | number | BN | any)
>(fieldType: T, val: any): FieldType<T> {
  const type:
    | FieldTypeEnum
    | RequestSchema
    | ((val: any) => string | boolean | number | BN | any) = fieldType;
  ...
  if (val == null && typeof type === 'number' && type >= 100) {
    return null;
  }
  ...
  if (type === FieldTypeEnum.Any || type === FieldTypeEnum.AnyOptional) {
    ...
    type === FieldTypeEnum.BooleanOptional
  ) {
    if (typeof val !== 'boolean') return;
    return val as any;
  } else if (
    type === FieldTypeEnum.Number ||
    type === FieldTypeEnum.NumberOptional
  ) {
    ...
    type === FieldTypeEnum.StringOptional
  ) {
...
export function verifySchema<T extends RequestSchema>(
  req: any,
  schema: T
): RequestSchemaResult<T> {
  ...
  for (let fieldName in schema) {
    ...
    const type: FieldTypeEnum | RequestSchema | ((val: any) => boolean) =
      schema[fieldName];
    if (typeof type === 'function') {
      const result = type(val);
