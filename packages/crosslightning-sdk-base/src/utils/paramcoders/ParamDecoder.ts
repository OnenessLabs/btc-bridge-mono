...
export class ParamDecoder implements IParamReader {
    ...
    while (leavesBuffer != null && leavesBuffer.length > 0) {
      if (this.frameHeader == null) {
        ...
        if (leavesBuffer.length <= requiredLen) {
          ...
          this.frameHeader = Buffer.concat([
            this.frameHeader,
    ...
    if (this.params[key] == null) {
      if (this.closed)
        return Promise.reject(
          new Error('Stream already closed without param received!')
        );
  ...
  async getParams<T extends RequestSchema>(
    schema: T
  ): Promise<RequestSchemaResult<T>> {
    ...
    for (let fieldName in schema) {
      ...
      if (type === FieldTypeEnum.Any || type === FieldTypeEnum.AnyOptional) {
        ...
        type === FieldTypeEnum.BN ||
        type === FieldTypeEnum.BNOptional
      ) {
