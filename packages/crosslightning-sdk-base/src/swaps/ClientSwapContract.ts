...
import { BitcoinRpc, BtcRelay } from '@onenesslabs/crosslightning-base';
import {
  findlnurl,
  getParams,
  LNURLPayParams,
...
import {
  RequestBody,
  streamingFetchWithTimeoutPromise,
} from '../utils/paramcoders/client/StreamingFetchPromise';
...
export class ClientSwapContract<T extends SwapData> {
  ...
  constructor(
    ...
    this.options.bitcoinNetwork =
      options.bitcoinNetwork || bitcoin.networks.testnet;
    ...
    this.options.maxExpectedOnchainSendSafetyFactor =
      options.maxExpectedOnchainSendSafetyFactor || 4;
    this.options.maxExpectedOnchainSendGracePeriodBlocks =
      options.maxExpectedOnchainSendGracePeriodBlocks || 12;
  }
  ...
  private verifyReturnedSignature(
    ...
    if (preFetchSignatureVerificationData == null)
      preFetchSignatureVerificationData = Promise.resolve(null);
    return tryWithRetries(
      () =>
        Promise.all([feeRatePromise, preFetchSignatureVerificationData]).then(
          ([feeRate, preFetchedData]) =>
            data.isPayIn()
              ? this.swapContract.isValidClaimInitAuthorization(
                  data,
                  parsedData.timeout,
                  parsedData.prefix,
              ...
              : this.swapContract.isValidInitAuthorization(
                  data,
                  parsedData.timeout,
                  parsedData.prefix,
                  parsedData.signature,
  ...
  private async verifyReturnedPrice(
    ...
    if (this.WBTC_ADDRESS != null) {
      ...
      const total = send
        ? amount.add(feeData.totalFee)
        : amount.sub(feeData.swapFee);
      ...
      if (
        this.swapPrice != null &&
        requiredBaseFee != null &&
        requiredFeePPM != null
      ) {
        const prefetchedPrice: BN =
          pricePrefetchPromise == null ? null : await pricePrefetchPromise;
        const isValidAmount = await (send
          ? this.swapPrice.isValidAmountSend(
              amount,
              requiredBaseFee,
              requiredFeePPM,
              data.getAmount().sub(feeData.networkFee),
              data.getToken(),
              abortSignal,
              prefetchedPrice
            )
          : this.swapPrice.isValidAmountReceive(
              amount,
              requiredBaseFee,
              requiredFeePPM,
              data.getAmount(),
  ...
  private async postWithRetries<T extends RequestSchema>(
    url: string,
    body: RequestBody,
    dataSchema: T,
    signal: AbortSignal,
    ...
    const { response, jsonBody, preFetchSignatureVerificationData } =
      await tryWithRetries(
        async () => {
          const { response, responseBody } =
            await streamingFetchWithTimeoutPromise(
              url,
              body,
              {
                ...
                ...(doPrefetchData &&
                this.swapContract.preFetchForInitSignatureVerification != null
          ...
          if (doPrefetchData) {
            if (
              ...
              _preFetchSignatureVerificationData = responseBody.signDataPrefetch
                .then((obj) => {
                  if (obj == null) return null;
                  return (
                    this.swapContract as any
                  ).preFetchForInitSignatureVerification(obj);
                })
                .catch((e) => {
                  console.error(e);
                  return null;
          ...
          return {
            ...
            preFetchSignatureVerificationData:
              _preFetchSignatureVerificationData,
          };
        },
        null,
        null,
  ...
  getOnchainSendTimeout(data: SwapData): BN {
    const tsDelta =
      (this.options.blocksTillTxConfirms + data.getConfirmations()) *
      BITCOIN_BLOCKTIME *
  ...
  private async getLNURL(
    ...
    if (MAIL_REGEX.test(str)) {
      ...
      if (shouldRetry) {
        response = await tryWithRetries(() =>
          fetchWithTimeout(
            scheme + '://' + domain + '/.well-known/lnurlp/' + username,
            {
              method: 'GET',
  ...
  isLNURL(str: string): boolean {
    return (
      findlnurl(str) != null || MAIL_REGEX.test(str) || this.isBareLNURL(str)
    );
  }
  ...
  async getLNURLType(
    str: string,
    shouldRetry?: boolean
  ): Promise<LNURLPay | LNURLWithdraw | null> {
  ...
  async payOnchain(
    ...
    const pricePreFetchPromise =
      this.swapPrice.preFetchPrice == null || requiredToken == null
        ? null
        : this.swapPrice
            .preFetchPrice(requiredToken, abortController.signal)
            .catch((e) => {
              console.error(e);
              return null;
            });

    const feeRatePromise: Promise<any> =
      this.swapContract.getInitPayInFeeRate == null ||
      requiredClaimerKey == null ||
      requiredToken == null
        ? null
        : tryWithRetries(() =>
            this.swapContract.getInitPayInFeeRate(
    ...
    const { parsedData, preFetchSignatureVerificationData } =
    ...
    const maxAllowedExpiryDelta: BN = new BN(
      confirmations +
        confirmationTarget +
        this.options.maxExpectedOnchainSendGracePeriodBlocks
    )
      .mul(new BN(this.options.maxExpectedOnchainSendSafetyFactor))
      .mul(new BN(this.options.bitcoinBlocktime));
    const currentTimestamp: BN = new BN(Math.floor(Date.now() / 1000));
    const maxAllowedExpiryTimestamp: BN = currentTimestamp.add(
      maxAllowedExpiryDelta
    );

    if (data.getExpiry().gt(maxAllowedExpiryTimestamp)) {
      console.error(
        'Expiry time returned: ' +
          data.getExpiry() +
          ' maxAllowed: ' +
          maxAllowedExpiryTimestamp
      );
      abortController.abort();
    ...
    return {
      ...
      expiry: await tryWithRetries(() =>
        preFetchSignatureVerificationData.then((preFetchedData) =>
          this.swapContract.getClaimInitAuthorizationExpiry(
            data,
            parsedData.timeout,
            parsedData.prefix,
            parsedData.signature,
  ...
  private async getAndVerifyPrFromLNURL(
    ...
    const queryParams =
      (payRequest.callback.includes('?') ? '&' : '?') + params.join('&');

    const response: Response = await tryWithRetries(() =>
      fetchWithTimeout(payRequest.callback + queryParams, {
        method: 'GET',
        timeout: this.options.getRequestTimeout,
      })
    );
    ...
    if (jsonBody.status === 'ERROR') {
      throw new RequestError(
        'LNURL callback error: ' + jsonBody.reason,
        response.status
      );
    }
      ...
      switch (successAction.tag) {
        case 'message':
          if (
            ...
            successAction.message.length > 144
          ) {
            throw new RequestError(
              'Invalid LNURL success action!',
              response.status
            );
          }
          break;
        case 'url':
          if (
            ...
            successAction.description.length > 144 ||
            successAction.url == null ||
            new URL(successAction.url).hostname !== payRequest.domain
          ) {
            throw new RequestError(
              'Invalid LNURL success action!',
              response.status
        ...
        case 'aes':
          if (
            ...
            successAction.ciphertext.length > 4096 ||
            !BASE64_REGEX.test(successAction.ciphertext) ||
            successAction.iv == null ||
            successAction.iv.length > 24 ||
            !BASE64_REGEX.test(successAction.iv)
          ) {
            throw new RequestError(
              'Invalid LNURL success action!',
              response.status
        ...
        default:
          throw new RequestError(
            'Unsupported LNURL success action!',
            response.status
          );
      }
    }
  ...
  private async payLightningLNURLExactIn(
    ...
    const { invoice: dummyInvoice, parsedPR: parsedDummyPR } =
      await this.getAndVerifyPrFromLNURL(
        payRequest,
        new BN(payRequest.minSendable).div(new BN(1000))
      );
    ...
    const responseInitial: Response = await tryWithRetries(
      () =>
        fetchWithTimeout(url + '/payInvoice', {
          method: 'POST',
          body: JSON.stringify({
            ...additionalParams,
            pr: dummyInvoice,
    ...
    if (responseInitial.status !== 200) {
      ...
      try {
        ...
        throw new RequestError(
          responseInitial.statusText,
          responseInitial.status
        );
      }
    ...
    if (jsonBody.signDataPrefetch != null) {
      if (
        (this.swapContract as any).preFetchForInitSignatureVerification != null
      ) {
        preFetchSignatureVerificationData = (this.swapContract as any)
          .preFetchForInitSignatureVerification(jsonBody.signDataPrefetch)
          .catch((e) => {
            console.error(e);
            return null;
          });
    ...
    const { invoice, successAction } = await this.getAndVerifyPrFromLNURL(
      payRequest,
      amountSats,
      comment
    );
  ...
  async payLightningLNURL(
    ...
    if (comment != null) {
      if (
        payRequest.commentAllowed == null ||
  ...
  async payLightning(
    ...
    if (expiryTimestamp == null)
      expiryTimestamp = new BN(Math.floor(Date.now() / 1000) + expirySeconds);
    ...
    if (pricePreFetchPromise == null)
      pricePreFetchPromise =
        this.swapPrice.preFetchPrice == null || requiredToken == null
          ? null
          : this.swapPrice.preFetchPrice(requiredToken, abortController.signal);
    ...
    const [_, parsedData] = await Promise.all([
      (async () => {
        const payStatus = await tryWithRetries(
          () =>
            this.swapContract.getPaymentHashStatus(
      ...
      (async () => {
        ...
        const {
          ...
          url + (exactIn ? '/payInvoiceExactIn' : '/payInvoice'),
          {
            ...additionalParams,
            reqId,
            pr: bolt11PayReq,
          ...
          abortController.signal,
          preFetchSignatureVerificationData == null
        );

        if (_preFetchSignatureVerificationData != null)
          preFetchSignatureVerificationData =
            _preFetchSignatureVerificationData;
    ...
    return {
      ...
      expiry: await tryWithRetries(() =>
        preFetchSignatureVerificationData.then((val) =>
          this.swapContract.getClaimInitAuthorizationExpiry(
            data,
            parsedData.timeout,
            parsedData.prefix,
  ...
  async getRefundAuthorization(
    ...
    const response: Response = await tryWithRetries(() =>
      fetchWithTimeout(
        url +
          '/getRefundAuthorization?paymentHash=' +
          encodeURIComponent(data.getHash()) +
          '&sequence=' +
          encodeURIComponent(data.getSequence().toString(10)),
        {
          method: 'GET',
          timeout: this.options.getRequestTimeout,
        }
    ...
    if (jsonBody.code === 20006) {
      ...
      if (txId != null) {
        const btcTx = await tryWithRetries(() =>
          ChainUtils.getTransaction(txId)
        );
        if (btcTx == null) {
        ...
        const foundVout = (btcTx as BitcoinTransaction).vout.find((e) =>
          this.swapContract
            .getHashForOnchain(
              Buffer.from(e.scriptpubkey, 'hex'),
              new BN(e.value),
              data.getEscrowNonce()
            )
            .equals(paymentHashBuffer)
        );
    ...
    if (jsonBody.code === 20000) {
      ...
      const isValidAuthorization = await tryWithRetries(async () => {
        try {
          await this.swapContract.isValidRefundAuthorization(
            data,
            jsonBody.data.timeout,
            jsonBody.data.prefix,
            jsonBody.data.signature
          );
          return null;
        } catch (e) {
      ...
      if (isValidAuthorization != null)
        throw new IntermediaryError(isValidAuthorization.message);
  ...
  async waitForRefundAuthorization(
    data: T,
    url: string,
    abortSignal?: AbortSignal,
  ...
  async receiveOnchain(
    ...
    const liquidityPromise: Promise<BN> =
      requiredToken == null || requiredOffererKey == null
        ? null
        : tryWithRetries(
            () =>
              this.swapContract.getIntermediaryBalance(
                requiredOffererKey,
                requiredToken
              ),
            null,
    ...
    const pricePrefetchPromise: Promise<BN> =
      requiredToken == null || this.swapPrice.preFetchPrice == null
        ? null
        : this.swapPrice
            .preFetchPrice(requiredToken, abortController.signal)
            .catch((e) => {
    ...
    const dummySwapData =
      requiredOffererKey == null || requiredToken == null
        ? null
        : await this.swapContract.createSwapData(
            ChainSwapType.CHAIN,
            requiredOffererKey,
            this.swapContract.getAddress(),
            requiredToken,
    ...
    const feeRatePromise: Promise<any> =
      this.swapContract.getInitFeeRate == null ||
      requiredOffererKey == null ||
      requiredToken == null
        ? Promise.resolve<any>(null)
        : tryWithRetries<any>(
            () =>
              this.swapContract.getInitFeeRate(
                requiredOffererKey,
                this.swapContract.getAddress(),
                requiredToken
              ),
    ...
    const initData: Promise<
      [
        BN,
        { blockheight: number; commitHash: string; chainWork: Buffer },
        number,
        BN
      ]
    > = Promise.all([
      tryWithRetries<BN>(
        () =>
          this.btcRelay
            .getFeePerBlock()
            .then((val) => val.mul(feeSafetyFactor || new BN(2))),
        null,
        null,
        abortController.signal
      ),
        ...
        () => {
          if ((this.swapContract as any).getRawClaimFee != null) {
            ...
            return this.swapContract
              .getClaimFee(dummySwapData)
              .then((value) => value.mul(feeSafetyFactor || new BN(2)));
          }
        },
        null,
        null,
    ...
    const { parsedData, preFetchSignatureVerificationData } =
      await this.postWithRetries(
        url + '/getAddress',
        {
          ...
          claimerBounty: initData.then(
            ([feePerBlock, btcRelayData, currentBtcBlock, addFee]) => {
              const currentBtcRelayBlock = btcRelayData.blockheight;
              const addBlock = Math.max(
                currentBtcBlock - currentBtcRelayBlock,
                0
              );
              return {
    ...
    const blocksDelta = tsDelta
      .div(new BN(this.options.bitcoinBlocktime))
      .mul(new BN(blockSafetyFactor));
    ...
    const lockingScript = bitcoin.address.toOutputScript(
      parsedData.btcAddress,
      this.options.bitcoinNetwork
    );

    const desiredHash = this.swapContract.getHashForOnchain(
      lockingScript,
      amount,
      new BN(0)
    );
    ...
    const [_, pricingInfo] = await Promise.all([
      ...
      (
        ...
        if (liquidity.lt(data.getAmount())) {
          throw new IntermediaryError(
            "Intermediary doesn't have enough liquidity"
          );
        }
      }),
      this.verifyReturnedPrice(
    ...
    return {
      ...
      expiry: await tryWithRetries(() =>
        preFetchSignatureVerificationData.then((val) =>
          this.swapContract.getInitAuthorizationExpiry(
            data,
            parsedData.timeout,
            parsedData.prefix,
            parsedData.signature,
  ...
  async postInvoiceToLNURLWithdraw(
    lnpr: string,
    k1: string,
    callbackUrl: string
  ): Promise<void> {
    const params = ['pr=' + lnpr, 'k1=' + k1];

    const queryParams =
      (callbackUrl.includes('?') ? '&' : '?') + params.join('&');

    const response: Response = await tryWithRetries(() =>
      fetch(callbackUrl + queryParams, {
        method: 'GET',
      })
    );
  ...
  async settleWithLNURLWithdraw(
    ...
    return {
      withdrawRequest,
      lnurlCallbackResult: this.postInvoiceToLNURLWithdraw(
        pr,
        withdrawRequest.k1,
  ...
  async receiveLightningLNURL(
    ...
    const resp = await this.receiveLightning(
      amount,
      url,
      requiredToken,
      requiredKey,
    ...
    const anyResp: any = resp;
    anyResp.lnurlCallbackResult = this.postInvoiceToLNURLWithdraw(
      resp.pr,
      withdrawRequest.k1,
      withdrawRequest.callback
    );
  ...
  async receiveLightning(
    ...
    const liquidityPromise: Promise<BN> =
      requiredToken == null || requiredKey == null
        ? null
        : tryWithRetries(
            () =>
              this.swapContract.getIntermediaryBalance(
    ...
    const { parsedData } = await this.postWithRetries(
      url + '/createInvoice',
      {
        ...additionalParams,
        ...
        token: requiredToken == null ? null : requiredToken.toString(),
        descriptionHash:
          descriptionHash == null ? null : descriptionHash.toString('hex'),
        exactOut,
        feeRate:
          feeRatePromise == null
            ? null
    ...
    if (
      ...
      decodedPR.tagsObject.purpose_commit_hash !==
        descriptionHash.toString('hex')
    ) {
      abortController.abort();
    ...
    if (exactOut) {
      ...
      if (
        !new BN(decodedPR.millisatoshis).div(new BN(1000)).eq(amountOrTokens)
      ) {
        abortController.abort();
        throw new IntermediaryError(
          'Invalid payment request returned, amount mismatch'
        );
      }
      amount = amountOrTokens;
    }
  ...
  async getPaymentAuthorization(
    ...
    const response: Response = await tryWithRetries(
      () =>
        fetchWithTimeout(
          url +
            '/getInvoicePaymentAuth?paymentHash=' +
            encodeURIComponent(paymentHash),
          {
            method: 'GET',
            signal: abortSignal,
            timeout: this.options.getRequestTimeout,
    ...
    if (jsonBody.code === 10000) {
      ...
      if (
        ...
        data.getSecurityDeposit().gt(maxSecurityDeposit)
      ) {
        ...
        (async () => {
          if (minOut != null) {
            ...
            if (
              ...
              const isValidAmount = await this.swapPrice.isValidAmountReceive(
                new BN(decodedPR.satoshis),
                requiredBaseFee,
                requiredFeePPM,
                data.getAmount(),
                requiredToken
      ...
      console.log(
        '[SmartChain.PaymentRequest] lightning payment hash: ',
        paymentHashInTx
      );
      ...
      console.log(
        '[SmartChain.PaymentRequest] Token amount: ',
