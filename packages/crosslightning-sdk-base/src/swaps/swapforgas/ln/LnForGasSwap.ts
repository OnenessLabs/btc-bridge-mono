...
export class LnForGasSwap<T extends SwapData> implements StorageObject {
  ...
  constructor(
    ...
    if (wrapperOrObject instanceof LnForGasWrapper) {
      ...
      this.outputAmount =
        wrapperOrObject.outputAmount == null
          ? null
          : new BN(wrapperOrObject.outputAmount);
      this.swapFee =
        wrapperOrObject.swapFee == null
          ? null
          : new BN(wrapperOrObject.swapFee);
  ...
  serialize(): any {
    return {
      ...
      url: this.url,
      outputAmount:
        this.outputAmount == null ? null : this.outputAmount.toString(10),
      swapFee: this.swapFee == null ? null : this.swapFee.toString(10),
      recipient: this.recipient,
    };
  }

  async getInvoiceStatus(abortSignal?: AbortSignal): Promise<{
    is_paid: boolean;
  ...
  async waitForPayment(
    ...
    while (abortSignal == null || !abortSignal.aborted) {
      const result = await this.getInvoiceStatus(abortSignal);
  ...
  save(): Promise<void> {
    return this.wrapper.storage.saveData(
