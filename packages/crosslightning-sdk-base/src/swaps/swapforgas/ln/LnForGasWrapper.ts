...
export class LnForGasWrapper<T extends SwapData> {
  ...
  constructor(
    ...
    if (options.getRequestTimeout == null)
      options.getRequestTimeout = 15 * 1000;
    if (options.postRequestTimeout == null)
      options.postRequestTimeout = 30 * 1000;
  ...
  async create(amount: BN, url: string): Promise<LnForGasSwap<T>> {
    ...
    const response: Response = await tryWithRetries(() =>
      fetchWithTimeout(
        url +
          ...
          encodeURIComponent(receiveAddress) +
          '&amount=' +
          encodeURIComponent(amount.toString(10)),
        {
          method: 'GET',
    ...
    const swap = new LnForGasSwap(
      this,
      jsonBody.data.pr,
      url,
      new BN(jsonBody.data.total),
      new BN(jsonBody.data.swapFee),
      receiveAddress
    );
  ...
  async init() {
    ...
    const processSwap: (swap: LnForGasSwap<T>) => Promise<boolean> = async (
      swap: LnForGasSwap<T>
    ) => {
          ...
          if (
            ...
            if (changed)
              return this.storage.saveData(
                swap.getPaymentHash().toString('hex'),
                swap
              );
          }
        })
