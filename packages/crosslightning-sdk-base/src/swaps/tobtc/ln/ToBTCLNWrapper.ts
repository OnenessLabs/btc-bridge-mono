...
export class ToBTCLNWrapper<T extends SwapData> extends IToBTCWrapper<T> {
  ...
  private calculateFeeForAmount(
    ...
    return new BN(
      overrideBaseFee || this.contract.options.lightningBaseFee
    ).add(
      amount
        .mul(new BN(overrideFeePPM || this.contract.options.lightningFeePPM))
        .div(new BN(1000000))
    );
  }
  ...
  async create(
    ...
    const result = await this.contract.payLightning(
      bolt11PayRequest,
      expirySeconds,
      fee,
      url,
  ...
  async createViaLNURL(
    ...
    if (exactIn && maxBaseFee == null) {
      ...
      fee = pricePreFetch
        .then((val) => {
          return this.contract.swapPrice.getFromBtcSwapAmount(
            new BN(this.contract.options.lightningBaseFee),
            requiredToken,
            null,
            val
          );
      ...
      fee = Promise.resolve(
        this.calculateFeeForAmount(amount, maxBaseFee, maxPPMFee)
      );
    }
