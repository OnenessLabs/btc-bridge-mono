...
export abstract class IToBTCSwap<T extends SwapData> extends ISwap {
  ...
  protected constructor(
    ...
    if (prefix != null || timeout != null || signature != null || url != null) {
      ...
      this.data =
        prOrObject.data != null
          ? new wrapper.swapDataDeserializer(prOrObject.data)
          : null;

      this.networkFee =
        prOrObject.networkFee == null ? null : new BN(prOrObject.networkFee);
      this.swapFee =
        prOrObject.swapFee == null ? null : new BN(prOrObject.swapFee);
  ...
  async commit(
    ...
    let txResult;
    try {
      txResult = await this.wrapper.contract.swapContract.initPayIn(
        this.data,
        this.timeout,
        this.prefix,
        this.signature,
  ...
  async txsCommit(skipChecks?: boolean): Promise<any[]> {
    ...
    let result: any[];
    try {
      result = await this.wrapper.contract.swapContract.txsInitPayIn(
        this.data,
        this.timeout,
        this.prefix,
        this.signature,
  ...
  waitForPayment(
    ...
    return Promise.race([
      new Promise<boolean>((resolve, reject) => {
        ...
        listener = (swap) => {
          if (swap.state === ToBTCSwapState.CLAIMED) {
            console.log(
              'IToBTCSwap: waitForPayment(): Triggered from on-chain listener!'
            );
            resolve(true);
            abortController.abort();
          }
      ...
      (async () => {
        const result = await this.wrapper.contract.waitForRefundAuthorization(
          this.data,
          this.url,
        ...
        console.log(
          'IToBTCSwap: waitForPayment(): Triggered from http request!'
        );
  ...
  canRefund(): boolean {
    return (
      this.state === ToBTCSwapState.REFUNDABLE ||
      this.wrapper.contract.swapContract.isExpired(this.data)
    );
  }
  ...
  async refund(
    ...
    if (this.wrapper.contract.swapContract.isExpired(this.data)) {
      txResult = await this.wrapper.contract.swapContract.refund(
        this.data,
        true,
        true,
        !noWaitForConfirmation,
      ...
      const res = await this.wrapper.contract.getRefundAuthorization(
        this.data,
        this.url
      );
      if (res.is_paid) {
      ...
      txResult =
        await this.wrapper.contract.swapContract.refundWithAuthorization(
          this.data,
          res.timeout,
          res.prefix,
          res.signature,
  ...
  async txsRefund(): Promise<any[]> {
    ...
    if (this.wrapper.contract.swapContract.isExpired(this.data)) {
      return await this.wrapper.contract.swapContract.txsRefund(
        this.data,
        true,
        true
      );
      ...
      return await this.wrapper.contract.swapContract.txsRefundWithAuthorization(
        this.data,
        res.timeout,
        res.prefix,
        res.signature,
  ...
  async refetchPriceData(): Promise<PriceInfoType> {
    ...
    const priceData = await this.wrapper.contract.swapPrice.isValidAmountSend(
      this.getOutAmount(),
