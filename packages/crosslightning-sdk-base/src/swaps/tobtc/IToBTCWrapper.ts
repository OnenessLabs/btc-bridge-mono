...
export abstract class IToBTCWrapper<T extends SwapData> {
   ...
   */
  protected async initWithConstructor(
    constructor: new (wrapper: any, data: any) => IToBTCSwap<T>
  ): Promise<void> {
    if (this.isInitialized) return;

    let eventQueue: SwapEvent<T>[] = [];
    this.swapData = await this.storage.loadSwapData<IToBTCSwap<T>>(
      this,
      constructor
    );
    ...
    const processSwap: (swap: IToBTCSwap<T>) => Promise<boolean> = async (
      swap: IToBTCSwap<T>
    ) => {
      if (swap.state === ToBTCSwapState.CREATED) {
        //Check if it's already committed
        const res = await tryWithRetries(() =>
        ...
        try {
          await tryWithRetries(
            () =>
              this.contract.swapContract.isValidClaimInitAuthorization(
                swap.data,
                swap.timeout,
                swap.prefix,
                swap.signature,
      ...
      if (swap.state === ToBTCSwapState.COMMITED) {
        ...
        if (res === SwapCommitStatus.COMMITED) {
          ...
          try {
            const refundAuth = await this.contract.getRefundAuthorization(
              swap.data,
              swap.url
            );
            if (refundAuth != null) {
