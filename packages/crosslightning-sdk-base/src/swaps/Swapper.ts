...
import { BtcRelay } from '@onenesslabs/crosslightning-base';
...
> extends EventEmitter {
  ...
  constructor(
    ...
    const clientSwapContract = new ClientSwapContract<T>(
      swapContract,
      swapDataConstructor,
      btcRelay,
      bitcoinRpc,
    ...
    this.tobtcln = new ToBTCLNWrapper<T>(
      options.storage?.toBtcLn ||
        new IndexedDBWrapperStorage(storagePrefix + 'Swaps-ToBTCLN'),
      clientSwapContract,
      chainEvents,
      swapDataConstructor,
      this
    ...
    this.tobtc = new ToBTCWrapper<T>(
      options.storage?.toBtc ||
        new IndexedDBWrapperStorage(storagePrefix + 'Swaps-ToBTC'),
      clientSwapContract,
      chainEvents,
      swapDataConstructor,
      this
    ...
    this.frombtcln = new FromBTCLNWrapper<T>(
      options.storage?.fromBtcLn ||
        new IndexedDBWrapperStorage(storagePrefix + 'Swaps-FromBTCLN'),
      clientSwapContract,
      chainEvents,
      swapDataConstructor,
      this
    ...
    this.frombtc = new FromBTCWrapper<T>(
      options.storage?.fromBtc ||
        new IndexedDBWrapperStorage(storagePrefix + 'Swaps-FromBTC'),
      clientSwapContract,
      chainEvents,
      swapDataConstructor,
      synchronizer,
    ...
    this.lnforgas = new LnForGasWrapper<T>(
      options.storage?.lnForGas ||
        new LocalStorageManager<LnForGasSwap<T>>(storagePrefix + 'LnForGas'),
      swapContract,
      options
    );
    ...
    if (options.intermediaryUrl != null) {
      this.intermediaryDiscovery = new IntermediaryDiscovery<T>(
        swapContract,
        options.registryUrl,
  ...
  async createSwap<S extends ISwap>(
    create: (candidate: Intermediary) => Promise<S>,
    amount: BN,
    ...
    if (!inBtc) {
      ...
      candidates = this.intermediaryDiscovery.getSwapCandidates(
        swapType,
        tokenAddress
      );
    } else {
  ...
  createTrustedLNForGasSwap(
    ...
    const useUrl =
      trustedIntermediaryUrl || this.options.defaultTrustedIntermediaryUrl;
    if (useUrl == null)
      throw new Error('No trusted intermediary URL specified!');
