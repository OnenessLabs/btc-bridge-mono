...
import { SwapCommitStatus, SwapData } from '@onenesslabs/crosslightning-base';
import { tryWithRetries } from '../../../utils/RetryUtils';
import { SignatureVerificationError } from '@onenesslabs/crosslightning-base';
...
export class FromBTCLNSwap<T extends SwapData> extends IFromBTCSwap<T> {
  ...
  constructor(
    ...
    if (typeof prOrObject === 'string') {
      ...
      this.requiredBaseFee =
        prOrObject.requiredBaseFee == null
          ? null
          : new BN(prOrObject.requiredBaseFee);
      this.requiredFeePPM =
        prOrObject.requiredFeePPM == null
          ? null
          : new BN(prOrObject.requiredFeePPM);
      this.expectedOut =
        prOrObject.expectedOut == null ? null : new BN(prOrObject.expectedOut);
   ...
   */
  getOutAmount(): BN {
    if (this.data != null && this.data.getAmount() != null)
  ...
  serialize(): any {
    ...
    partiallySerialized.requiredBaseFee =
      this.requiredBaseFee == null ? null : this.requiredBaseFee.toString(10);
    partiallySerialized.requiredFeePPM =
      this.requiredFeePPM == null ? null : this.requiredFeePPM.toString(10);
    partiallySerialized.expectedOut =
      this.expectedOut == null ? null : this.expectedOut.toString(10);
  ...
  async waitForPayment(
    ...
    if (this.lnurl != null && !this.prPosted) {
      this.callbackPromise = this.wrapper.contract.postInvoiceToLNURLWithdraw(
        this.pr,
        this.lnurlK1,
        this.lnurlCallback
      );
  ...
  async settleWithLNURLWithdraw(
    ...
    const result = await this.getWrapper().contract.settleWithLNURLWithdraw(
  ...
  async commit(
    ...
    let txResult;
    try {
      txResult = await this.wrapper.contract.swapContract.init(
        this.data,
        this.timeout,
        this.prefix,
        this.signature,
        null,
        !noWaitForConfirmation,
        skipChecks,
        abortSignal,
        this.feeRate
      );
  ...
  async txsCommit(skipChecks?: boolean): Promise<any[]> {
    ...
    let txs: any[];
    try {
      txs = await this.wrapper.contract.swapContract.txsInit(
        this.data,
        this.timeout,
        this.prefix,
        this.signature,
  ...
  waitTillCommited(abortSignal?: AbortSignal): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const intervalWatchdog = setInterval(() => {
        this.wrapper.contract.swapContract
          .getCommitStatus(this.data)
          .then((status) => {
      ...
      abortController.signal.addEventListener('abort', () =>
        clearInterval(intervalWatchdog)
      );
  ...
  async claim(
    ...
    const txResult = await this.wrapper.contract.swapContract.claimWithSecret(
      this.data,
  ...
  async txsClaim(): Promise<any[]> {
    ...
    return await this.wrapper.contract.swapContract.txsClaimWithSecret(
      this.data,
      this.secret.toString('hex'),
      true,
      true
  ...
  async commitAndClaim(
    ...
    if (this.state === FromBTCLNSwapState.CLAIM_COMMITED) {
      return [null, await this.claim(false, abortSignal)];
    }
    ...
    try {
      txResult =
        await this.wrapper.contract.swapContract.initAndClaimWithSecret(
          this.data,
          this.timeout,
          this.prefix,
          this.signature,
  ...
  async txsCommitAndClaim(skipChecks?: boolean): Promise<any[]> {
    ...
    let initTxs: any[];
    try {
      initTxs = await this.wrapper.contract.swapContract.txsInit(
        this.data,
        this.timeout,
        this.prefix,
        this.signature,
    ...
    const claimTxs =
      await this.wrapper.contract.swapContract.txsClaimWithSecret(
        this.data,
        this.secret.toString('hex'),
        true,
        true
