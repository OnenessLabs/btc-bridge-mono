...
export class FromBTCSwap<T extends SwapData> extends IFromBTCSwap<T> {
  ...
  async waitForPayment(
    ...
    const result = await ChainUtils.waitForAddressTxo(
      this.address,
      this.getTxoHash(),
  ...
  async getBitcoinPayment(): Promise<{
    ...
    const result = await ChainUtils.checkAddressTxos(
      this.address,
      this.getTxoHash()
    );
  ...
  canCommit(): boolean {
    ...
    if (
      expiry
        ...
        .lt(new BN(this.wrapper.contract.options.minSendWindow))
    ) {
      return false;
    }
    return true;
  ...
  async claim(
    ...
    const txResult = await this.wrapper.contract.swapContract.claimWithTxData(
      this.data,
      ...
      this.vout,
      null,
      (this.wrapper as FromBTCWrapper<T>).synchronizer,
      true,
      !noWaitForConfirmation,
      abortSignal
    );
  ...
  async txsClaim(): Promise<any[]> {
    ...
    return await this.wrapper.contract.swapContract.txsClaimWithTxData(
      this.data,
      txData.status.block_height,
      {
        blockhash: txData.status.block_hash,
  ...
  getTxoHash(): Buffer {
    const parsedOutputScript = bitcoin.address.toOutputScript(
      this.address,
      this.wrapper.contract.options.bitcoinNetwork
    );
  ...
  getQrData(): string {
    return this.state === FromBTCSwapState.PR_CREATED
      ? null
      : 'bitcoin:' +
          this.address +
          '?amount=' +
          encodeURIComponent((this.amount.toNumber() / 100000000).toString(10));
  }
  ...
  getTimeoutTime(): number {
    return (
      this.wrapper.contract.getOnchainSendTimeout(this.data).toNumber() * 1000
    );
  }
