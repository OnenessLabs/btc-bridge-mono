...
export class FromBTCWrapper<T extends SwapData> extends IFromBTCWrapper<T> {
  ...
  async create(
    ...
    const result = await this.contract.receiveOnchain(
      amount,
      url,
      requiredToken,
      requiredKey,
  ...
  async init() {
    ...
    this.swapData = await this.storage.loadSwapData<FromBTCSwap<T>>(
      this,
      FromBTCSwap
    );
    ...
    const processEvent = async (events: SwapEvent<T>[]) => {
      for (let event of events) {
        ...
        const swap: FromBTCSwap<T> = this.swapData[
          paymentHash
        ] as FromBTCSwap<T>;
        ...
        if (event instanceof InitializeEvent) {
          if (swap.state === FromBTCSwapState.PR_CREATED) {
            if (swap.data != null) {
              try {
                if (!swap.data.getSequence().eq(event.sequence))
        ...
        if (event instanceof ClaimEvent) {
          if (
            swap.state === FromBTCSwapState.PR_CREATED ||
            swap.state === FromBTCSwapState.CLAIM_COMMITED ||
    ...
    const processSwap: (swap: FromBTCSwap<T>) => Promise<boolean> = async (
      swap: FromBTCSwap<T>
    ) => {
      if (swap.state === FromBTCSwapState.PR_CREATED) {
        ...
        if (status === SwapCommitStatus.COMMITED) {
          ...
          const tx = await ChainUtils.checkAddressTxos(
            swap.address,
            swap.getTxoHash()
          );
