...
import { BtcStoredHeader } from '@onenesslabs/crosslightning-base';
import { BigNumber } from 'ethers';
import { StatePredictorUtils } from '@onenesslabs/crosslightning-base/dist';
...
export class EVMBtcStoredHeader implements BtcStoredHeader<EVMBtcHeader> {
  ...
  getChainWork(): Buffer {
    return Buffer.from(
      this.chainWork.toHexString().substring(2).padStart(64, '0'),
      'hex'
    );
  }

  getHeader(): EVMBtcHeader {
    const version = reverseBigNumber(
      this.data1.shr(224).and(BigNumber.from(0xffffffff))
    );
    const nbits = reverseBigNumber(
      this.data1.shr(192).and(BigNumber.from(0xffffffff))
    );
    const nonce = reverseBigNumber(
      this.data1.shr(160).and(BigNumber.from(0xffffffff))
    );
    const timestamp = this.data2.and(BigNumber.from(0xffffffff));
    return new EVMBtcHeader({
      version: version.toNumber(),
      reversedPrevBlockhash: Buffer.from(
        this.reversedPrevBlockHash.substring(2),
        'hex'
      ),
  ...
  computeNext(header: EVMBtcHeader): EVMBtcStoredHeader {
    ...
    const data1: BigNumber = reverseBigNumber(BigNumber.from(header.version))
      .and(BigNumber.from('0xFFFFFFFF'))
      .shl(224)
      .or(
        reverseBigNumber(BigNumber.from(header.nbits))
          .and(BigNumber.from('0xFFFFFFFF'))
          .shl(192)
      )
      .or(
        reverseBigNumber(BigNumber.from(header.nonce))
          .and(BigNumber.from('0xFFFFFFFF'))
          .shl(160)
      )
      .or(
        BigNumber.from(lastDiffAdjustment)
          .and(BigNumber.from('0xFFFFFFFF'))
          .shl(128)
      )
      .or(BigNumber.from(blockheight).and(BigNumber.from('0xFFFFFFFF')).shl(96))
      .or(
        BigNumber.from(prevBlockTimestamps[0])
          .and(BigNumber.from('0xFFFFFFFF'))
          .shl(64)
      )
      .or(
        BigNumber.from(prevBlockTimestamps[1])
          .and(BigNumber.from('0xFFFFFFFF'))
          .shl(32)
      )
      .or(
        BigNumber.from(prevBlockTimestamps[2]).and(BigNumber.from('0xFFFFFFFF'))
      );
    ...
    for (let i = 0; i < 7; i++) {
      data2 = data2.or(
        BigNumber.from(prevBlockTimestamps[3 + i])
          .and('0xFFFFFFFF')
          .shl((7 - i) * 32)
      );
    }
    ...
    return new EVMBtcStoredHeader({
      ...
      reversedPrevBlockHash:
        '0x' + header.reversedPrevBlockhash.toString('hex'),
